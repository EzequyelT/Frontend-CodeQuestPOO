...
import { ArrowLeft, Trophy, Clock, Sword, Map, Filter, Crown, Star, ChevronDown, Sparkles } from "lucide-react";
...
function LeaderRow({ entry, pos, isMe, timeKey = "melhorTempo", showContext = false }) {
    ...
    return (
        ...
        >
            ...
            <Avatar nome={entry.nome} avatar={entry.avatar} isMe={isMe} />
            ...
            <div className="flex flex-col flex-1 min-w-0">
                <span className={`font-black text-sm truncate tracking-wide ${isMe ? "text-emerald-400" : isTop3 ? "text-white" : "text-neutral-300"}`}>
                    {entry.nome}
                    {isMe && <span className="ml-1 text-[10px] font-extrabold text-emerald-400 uppercase tracking-widest bg-emerald-500/10 px-1.5 py-0.5 rounded"> (tu)</span>}
                </span>
        ...
        return (
            <div className={`flex flex-col items-center gap-2 flex-1 max-w-[160px] animate-fade-in-up`} style={{ animationDelay }}>
                <div className="relative group">
                    ...
                    <Avatar nome={entry.nome} avatar={entry.avatar} size={pos === 0 ? 64 : 52} isMe={isMe} />
                </div>
                ...
                >
                    ...
                    <span className="text-4xl filter drop-shadow-md select-none transform group-hover:scale-110 transition-transform">{rank.badge}</span>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="relative flex-1 min-w-[140px]">
            ...
            >
                ...
                {options.map(o => (
                    <option key={o.value} value={o.value} className="bg-neutral-950 text-neutral-200">{o.label}</option>
                ))}
            </select>
...
function EmptyState({ message }) {
    return (
        <div className="text-center py-16 border-2 border-dashed border-neutral-800/60 rounded-2xl bg-neutral-900/10 backdrop-blur-sm animate-pulse">
            ...
            <p className="text-neutral-500 text-xs mt-1.5">Completa desafios na campanha para deixar a tua marca aqui!</p>
        </div>
    );
}
...
export default function LeaderBoard() {
    ...
    useEffect(() => {
        async function bootstrap() {
            try {
                ...
                const global = await getLeaderBoardBestTime();
    ...
    useEffect(() => {
        if (!selectedMapa) { setNiveis([]); setDesafios([]); setSelectedNivel(null); setSelectedDesafio(null); return; }
    ...
    useEffect(() => {
        if (!selectedNivel) { setDesafios([]); setSelectedDesafio(null); return; }
        setSelectedDesafio(null);
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-neutral-400 text-xs mt-0.5 font-medium flex items-center gap-1.5">
                                    <Sparkles size={12} className="text-amber-400" /> Classificação em tempo real da campanha
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 animate-fade-in">
                            <div className="flex items-center justify-between px-2">
                                <h2 className="text-neutral-400 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                                    <Sword size={13} className="text-amber-500" /> Melhores Tempos Gerais
                                </h2>
                        ...
                        <div className="space-y-4 animate-fade-in">
                            <div className="bg-neutral-900/30 border border-neutral-900 p-5 rounded-3xl backdrop-blur-md shadow-lg">
                                <p className="text-neutral-400 text-[10px] uppercase tracking-widest font-black mb-4 flex items-center gap-2">
                                    <Filter size={12} className="text-amber-500" /> Filtros de Classificação
                                </p>
                                ...
                                {userProgress && !selectedMapa && (
                                    <p className="text-neutral-400 text-[11px] font-medium mt-4 bg-neutral-950/60 p-2.5 rounded-xl border border-neutral-900 flex items-center gap-2">
                                        <span className="animate-pulse text-amber-400">💡</span> O teu progresso atual: <span className="text-amber-400 font-bold">Mapa {userProgress.mapaId}</span> · <span className="text-neutral-300 font-bold">Nível {userProgress.nivelId}</span>
                                    </p>
                                )}
                            ...
                            {filteredLoading ? (
                                ...
                                filteredEntries.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between px-2">
                                            <h2 className="text-neutral-400 text-xs font-black uppercase tracking-widest flex items-center gap-2">
                                                <Trophy size={13} className="text-amber-500" /> Resultados do Filtro
                                            </h2>
                                            <span className="bg-neutral-900 text-neutral-400 text-[10px] font-bold px-2.5 py-1 rounded-full border border-neutral-800">
